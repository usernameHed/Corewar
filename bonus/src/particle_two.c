/*
** particle_two.c for  in /home/belfio_u/rendu/CPE/CPE_2014_corewar/bonus/src
** 
** Made by ugo belfiore
** Login   <belfio_u@epitech.net>
** 
** Started on  Sat Apr  4 00:08:24 2015 ugo belfiore
** Last update Sat Apr 11 17:50:25 2015 ugo belfiore
*/
...
static void	two_modif_color_bis(t_data *d, int i, int j)
{
  if (d->arene[i] == '3' && d->ow[1].m.tab[j].color_particle != 3)
    {
      (d->ch.start == 2) ? d->ow[1].m.tab[j].XPos = d->ch.x_cible[3] : 1;
      (d->ch.start == 2) ? d->ow[1].m.tab[j].YPos = d->ch.y_cible[3] : 1;
      (d->ch.start != 2) ? d->ow[1].m.tab[j].color_particle = 3 : 1;
      (d->ch.start != 2) ? d->ow[1].m.tab[j].timer = 20 : 1;
    }
  if (d->arene[i] == '4' && d->ow[1].m.tab[j].color_particle != 4)
    {
      (d->ch.start == 2) ? d->ow[1].m.tab[j].XPos = d->ch.x_cible[4] : 1;
      (d->ch.start == 2) ? d->ow[1].m.tab[j].YPos = d->ch.y_cible[4] : 1;
      (d->ch.start != 2) ? d->ow[1].m.tab[j].color_particle = 4 : 1;
      (d->ch.start != 2) ? d->ow[1].m.tab[j].timer = 20 : 1;
    }
}

static void	two_modif_color(t_data *d, int i, int j)
{
  if (d->arene[i] == '1' && d->ow[1].m.tab[j].color_particle != 1)
    {
      (d->ch.start == 2) ? d->ow[1].m.tab[j].XPos = d->ch.x_cible[1] : 1;
      (d->ch.start == 2) ? d->ow[1].m.tab[j].YPos = d->ch.y_cible[1] : 1;
      (d->ch.start != 2) ? d->ow[1].m.tab[j].color_particle = 1 : 1;
      (d->ch.start != 2) ? d->ow[1].m.tab[j].timer = 20 : 1;
    }
  if (d->arene[i] == '2' && d->ow[1].m.tab[j].color_particle != 2)
    {
      (d->ch.start == 2) ? d->ow[1].m.tab[j].XPos = d->ch.x_cible[2] : 1;
      (d->ch.start == 2) ? d->ow[1].m.tab[j].YPos = d->ch.y_cible[2] : 1;
      (d->ch.start != 2) ? d->ow[1].m.tab[j].color_particle = 2 : 1;
      (d->ch.start != 2) ? d->ow[1].m.tab[j].timer = 20 : 1;
    }
  two_modif_color_bis(d, i, j);
}

static void	two_modif(t_data *d)
{
  int	i;
  int	j;
  int	end;
  ...
  while (i < end)
    {
      if (d->arene[i] == '0' && d->ow[1].m.tab[j].color_particle != 0)
	{
	  d->ow[1].m.tab[j].color_particle = 0;
	  d->ow[1].m.tab[j].timer = 20;
	}
      two_modif_color(d, i, j);
      i++;
      j++;
    }
}
...
static void	two_big_calcule(t_data *d, int j)
{
  double	tmp_a;
  int		tmpj;
  int		tmpx;
  ...
  tmp_a = 0;
  if (d->ch.style == 1)
    tmp_a = (atan2(((C20 C21) * -1), (C22 C23) * 1)) * (180.0 / PI) + 180;
  else
    {
      while (tmpj >= 128)
	{
      ...
      xx = 57 + (tmpj * 11);
      yy = 278 + tmpx;
      tmp_a = (atan2(((d->ow[1].m.tab[j].YPos - yy)* -1),
		     (d->ow[1].m.tab[j].XPos - xx) * 1)) * (180.0 / PI) + 180;
    }
  tmp_a_two(d, j, tmp_a);
}
...
void		particle_two(t_data *d)
{
  ...
  two_modif(d);
  j = -1;
  while (++j < d->ow[1].m.y_file)
    {
      d->ow[1].m.nb_par[0] += (d->ow[1].m.tab[j].color_particle == 1) ? 1 : 0;
      d->ow[1].m.nb_par[1] += (d->ow[1].m.tab[j].color_particle == 2) ? 1 : 0;
      d->ow[1].m.nb_par[2] += (d->ow[1].m.tab[j].color_particle == 3) ? 1 : 0;
      d->ow[1].m.nb_par[3] += (d->ow[1].m.tab[j].color_particle == 4) ? 1 : 0;
      if ((d->ch.style == 1 && !(random() % 3)) || d->ch.style == -1)
  	{
  	  if (d->ow[1].m.tab[j].timer > 0)
  	    d->ow[1].m.tab[j].timer--;
  	  if (!(random() % 3))
  	    d->ow[1].m.tab[j].zzz--;
  	  if (d->ow[1].m.tab[j].zzz < 0)
  	    d->ow[1].m.tab[j].zzz = 30;
	  two_big_calcule(d, j);
  	}
      d->ow[1].m.tab[j].XPos += (C24) ? d->ow[1].m.tab[j].XVel : 0;
      d->ow[1].m.tab[j].YPos += (C25) ? d->ow[1].m.tab[j].YVel : 0;
      aff_particule_two(d, j);
    }
}
